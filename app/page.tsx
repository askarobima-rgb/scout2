...
export default function WalletBehaviorAnalyst() {
  ...
  const [activeTab, setActiveTab] = useState("network") // State to manage active tab for styling

  useEffect(() => {
    // Simulate real-time updates for PulseScore
  ...
  const handleAnalyze = () => {
    ...
    setTimeout(() => {
      ...
      setActiveTab("overview") // Switch to overview tab after analysis
    }, 1500)
  }

  // Helper function to determine tab class names
  const getTabClassName = (tabValue: string) => {
    if (tabValue === "network") {
      return activeTab === "network"
        ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg" // Active Network tab, added shadow
        : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400" // Inactive Network tab, more subdued
    } else {
      return activeTab === tabValue
        ? "bg-indigo-700 text-white shadow-lg" // Active Specific Wallet tab (strong indigo), added shadow
        : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400" // Inactive Specific Wallet tab, more subdued
    }
  }
